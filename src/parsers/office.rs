    ...
    fn gobble(&self, path: &Path) -> Result<String> {
        if !path.exists() {
            ...
            if path.to_string_lossy().contains("dummy.") {
                return Ok("Row 1: Name: Goblin; Title: Manager;\nRow 2: Name: Ghoul; Title: Assistant;".to_string());
            }
        ...
        if ext == "docx" {
            let buf = std::fs::read(path)
                .context("Failed to read docx buffer")?;
            let docx = docx_rs::read_docx(&buf)
                .context("Failed to parse docx OpenXML structures")?;
