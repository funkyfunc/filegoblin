...
use oauth2::{
    ...
    Scope, TokenResponse, TokenUrl, basic::BasicClient, reqwest::http_client,
};
...
impl GoogleGobbler {
    fn extract_file_id(&self, url: &str) -> Option<String> {
        ...
        for segment in segments {
            if segment == "d" {
                return parsed
                    .path_segments()?
                    .nth(parsed.path_segments()?.position(|s| s == "d")? + 1)
                    .map(|s| s.to_string());
            }
        }
    ...
    fn refresh_or_get_token(&self) -> Result<Option<String>> {
        ...
        if let (Some(token), Some(expires_at), Some(refresh)) = (
            &creds.google_access_token,
            creds.google_token_expires_at,
...
impl Gobble for GoogleGobbler {
    ...
    fn gobble_str(&self, url: &str, _flags: &crate::cli::Cli) -> Result<String> {
        let file_id = self
            .extract_file_id(url)
            .context("Could not extract Google Drive/Docs file ID from URL")?;

        let token = self.refresh_or_get_token()?.context(
            "No Google access token found. Please run `filegoblin --google-login` to authenticate.",
        )?;
        ...
        if !meta_res.status().is_success() {
            anyhow::bail!(
                "Google API returned status {} when fetching metadata.",
                meta_res.status()
            );
        }
            ...
            || mime_type.starts_with("application/vnd.google-apps")
        {
            ...
            if !export_res.status().is_success() {
                anyhow::bail!(
                    "Google API failed to export document. Status: {}",
                    export_res.status()
                );
            }
            ...
            if !download_res.status().is_success() {
                anyhow::bail!(
                    "Google API failed to download file. Status: {}",
                    download_res.status()
                );
            }
            ...
            Ok(format!(
                "File downloaded to temporary location: {}\n\nRun `filegoblin {}` to parse binary contents locally.",
...
fn build_google_oauth_client() -> BasicClient {
    ...
    let client_id = std::env::var("GOOGLE_CLIENT_ID").unwrap_or_else(|_| {
        "1046465451998-vt1kdb6487hsvntg1s9h1uijsdj8412s.apps.googleusercontent.com".to_string()
    }); // Public dummy / user injected
...
pub fn handle_google_login() -> Result<()> {
    ...
    eprintln!(
        "{}",
        "🌐 Initiating Google OAuth 2.0 PKCE Login...".truecolor(0, 200, 255)
    );
    ...
    eprintln!(
        "\n{}\n",
        "Press ENTER to open your browser and authorize 'filegoblin' for Google Workspace access."
            .bold()
    );
    eprintln!(
        "If the browser doesn't open automatically, navigate to this URL:\n{}\n",
    ...
    let server = Server::http("127.0.0.1:7890").map_err(|e| {
        anyhow::anyhow!(
            "Failed to bind to localhost:7890. Is the port in use? {}",
            e
        )
    })?;
    ...
    for request in server.incoming_requests() {
        ...
        if url.starts_with("/callback") {
            ...
            let response = Response::from_string(
                "Google Auth Success! You can now close this tab and return to the terminal.",
            );
    ...
    eprintln!(
        "\n{}",
        "✨ GEMINI SHARE LINK INGESTION SETUP ✨"
            .truecolor(255, 191, 0)
            .bold()
    );
    eprintln!(
        "To ingest Gemini share links, we need your active Google session cookie (`__Secure-1PSID`)."
    );
    ...
    eprintln!(
        "{} Google login flow complete! Credentials saved to ~/.config/filegoblin/credentials.json",
        "✅".green()
    );
